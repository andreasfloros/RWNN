...
def main():
    ...
    lambd = 0.23
    model.module.transform_net.nenet.override_flag = True
    ...
    for f in files_source:
        ...
        # degrade
        mask = pngs_to_tensors(f'kernels_masks/mask{256 if img_H_tensor.shape[-1] == 256 else 512}.png').cuda() == 0.

        x = mask * (img_H_tensor + (opt.test_nl / 255.) * torch.randn_like(img_H_tensor))
        y = x

        # initialize iteration count and constants
        i = 0
        stdNv_ = max(1., opt.test_nl) / 255.
        stdNv = (255. / 255.)

        save_tensor(x[0, ...].cpu(), f"examples/pnp/inpaint/{img_index}_degraded_masked.png")

        while stdNv > stdNv_:
            model.module.transform_net.nenet.override = stdNv * 0.9
        ...
        save_tensor(x[0, ...].cpu(), f"examples/pnp/inpaint/{img_index}_out_masked.png")
    ...
    print(f'PSNR for mask:', psnr_test / img_index)
